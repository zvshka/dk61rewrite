...
export async function quoteMessage(
  initiator: MessageContextMenuCommandInteraction | Message,
  background?: Attachment
) {
  // Получаем полную информацию о сообщении.
  const targetMessage =
    initiator instanceof MessageContextMenuCommandInteraction
      ? initiator.targetMessage
      : await initiator.fetchReference();
  ...
  const lines = text
    .split('\n')
    .map(row => getLines(textCtx, row, 880, `${mainTextSize}px Google Sans Italic`))
    .flat();
...
function getLines(ctx: SKRSContext2D, text: string, maxWidth: number, font: string) {
  ...
  for (let i = 1; i < words.length; i++) {
    ...
    const width = ctx.measureText(toMeasure.replaceAll(/<?(a:|:)\w*:(\d*)>/gi, 'EM')).width;
    if (width < maxWidth) {
      currentLine += ` ${word}`;
    } else {
